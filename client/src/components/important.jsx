...
export default function App() {
  ...
  const getimpTasksQuery = useQuery({
    queryKey: ["getimpTasksQuery"],
    queryFn: getimpTasks,
  });

  // Handle the query state
  useEffect(() => {
    if (getimpTasksQuery.isSuccess) {
      // console.log("Data fetched successfully:", getallTasksQuery.data);
      if (getimpTasksQuery.data && Array.isArray(getimpTasksQuery.data.data)) {
        const formattedData = getimpTasksQuery.data.data.map((item) => ({
          ...item,
        ...
        console.error(
          ...
          getimpTasksQuery.data,
        );
        setData([]);
      }
    }
    if (getimpTasksQuery.isError) {
      const error = getimpTasksQuery.error;
      message.error(
    ...
    getimpTasksQuery.isSuccess,
    getimpTasksQuery.isError,
    getimpTasksQuery.data,
    getimpTasksQuery.error,
  ]);
  ...
  const updateTaskMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries(["getimpTasksQuery"]);
    },
    onError: (error) => {
      message.error(
        error.response?.data?.message || error.message || "An error occurred",
  ...
  const deleteTaskMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries(["getimpTasksQuery"]); // Invalidate the query to refetch data
    },
    onError: (error) => {
      message.error(
        error.response?.data?.message ||
    ...
    <div style={styles.section}>
      <div style={styles.container}>
        <Table
          ...
          columns={columns}
          dataSource={data}
          loading={getimpTasksQuery.isLoading}
