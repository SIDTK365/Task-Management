...
export default function App() {
  ...
  const getincompleteTasksQuery = useQuery({
    queryKey: ["getincompleteTasksQuery"],
    queryFn: getincompleteTasks,
  });

  // Handle the query state
  useEffect(() => {
    if (getincompleteTasksQuery.isSuccess) {
      // console.log("Data fetched successfully:", getallTasksQuery.data);
      if (
        getincompleteTasksQuery.data &&
        Array.isArray(getincompleteTasksQuery.data.data)
      ) {
        const formattedData = getincompleteTasksQuery.data.data.map((item) => ({
          ...item,
    ...
    if (getincompleteTasksQuery.isError) {
      const error = getincompleteTasksQuery.error;
      message.error(
    ...
    getincompleteTasksQuery.isSuccess,
    getincompleteTasksQuery.isError,
    getincompleteTasksQuery.data,
    getincompleteTasksQuery.error,
  ]);
  ...
  const updateTaskMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries(["getincompleteTasksQuery"]);
    },
    onError: (error) => {
      message.error(
        error.response?.data?.message || error.message || "An error occurred",
  ...
  const deleteTaskMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries(["getincompleteTasksQuery"]); // Invalidate the query to refetch data
    },
    onError: (error) => {
      message.error(
        error.response?.data?.message ||
    ...
    <div style={styles.section}>
      <div style={styles.container}>
        <Table
          ...
          columns={columns}
          dataSource={data}
          loading={getincompleteTasksQuery.isLoading}
